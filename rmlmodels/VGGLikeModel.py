...
def VGGLikeModel(weights='ResNetLike-125k.wts.h5',
    ...
    model.add(Dense(128, activation='selu', kernel_initializer='he_normal', name="dense1",kernel_regularizer=regularizers.l2(0.01)))
    model.add(Dropout(dr))

    model.add(Dense(128, activation='selu', kernel_initializer='he_normal', name="dense2"))
    model.add(Dropout(dr))

    model.add(Dense(classes, kernel_initializer='he_normal', name="dense3",kernel_regularizer=regularizers.l2(0.01)))
...
from keras.layers import Input,Dense,Conv1D,MaxPool1D,BatchNormalization,Dropout#,ReLU
...
def ConvBNReluUnit(input,kernel_size = 8,index = 0):
    ...
    #3333333333333x = ReLU(name='conv{}-relu'.format(index + 1))(x)
